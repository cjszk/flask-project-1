...
mode_list = ["start", "stop", "restart", "status", "list", "info", "update",
             ...
             "repolist", "repo-pkgs", "discover", "verify-ami","test-cred",
...
try:
  ...
  if p_mode == "test-cred":
    parser = argparse.ArgumentParser()
    parser.add_argument('--cloud', required=True, choices=["aws"])
    parser.add_argument('--credentials', help="credentials json required", required=True)
    known_args, unknown_args = parser.parse_known_args(args)
    args = unknown_args
    try:
      credentials = json.loads(known_args.credentials)
    except:
      util.exit_message("--credentials should be json", 1, isJSON)
    if known_args.cloud == 'aws' and not set(("aws_access_key_id", "aws_secret_access_key")).issubset(credentials):
      util.exit_message("aws_access_key_id, aws_secret_access_key are required in credentials.", 1, isJSON)
    response = dbaas.testconnection(isJSON,isVERBOSE, p_cloud=known_args.cloud, credentials=credentials)
    util.exit_message(response['msg'], response["code"], isJSON)
  ...
  if p_mode == "metalist":
    ...
    if len(args) == 3:
      if args[2] in ("rds-versions", "vpc-list", "instance-class", "images", "res-group",
                     "instance-type","storage-accounts","keypairs", "sec-group"):
        if known_args.cloud not in ['aws']:
          util.exit_message("Missing or invalid cloud parameter. Possible values are aws", 1, isJSON)
