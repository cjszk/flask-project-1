...
class AwsConnection(object):
    ...
    def create_db(self, p_dict):
        ...
        try:
            ...
            response_json['state'] = 'failed'
            response_json['msg'] = msg1 + str(e)
    ...
    def create_vm(self, p_dict):
        ...
        for d in p_dict:
            ...
            if d.get("keyname"):
                ec2_params['KeyName'] = d.get("keyname")
            ...
            if d.get("security_group"):
                ec2_params["SecurityGroupIds"] = [d["security_group"]]
            ...
            if d.get("volume_size") and d.get("storage_type"):
                ec2_params['BlockDeviceMappings'] = [
                    {
                        'DeviceName': '/dev/sda1',
                        'Ebs': {
                            'VolumeSize': d['volume_size'],
                            'VolumeType': d['storage_type']
                        }
                    }
                ]
    ...
    def security_groups(self, region=None):
        ...
        if self.instance_type == 'ec2':
            results = self.client.describe_security_groups()
            for res in results.get('SecurityGroups',[]):
                group = {}
                group['groupName'] = res.get('GroupName','')
                group['groupId'] = res.get('GroupId','')
                group['vpcId'] = res.get('VpcId','')
                result.append(group)
        return result
    ...
    def key_pairs(self, region=None):
        result = []
        if self.instance_type == 'ec2':
            results = self.client.describe_key_pairs()
            for res in results.get('KeyPairs',[]):
                group = {}
                group['keyName'] = res.get('KeyName')
                group['keyFingerprint'] = res.get('KeyFingerprint','')
                result.append(group)
        return result
    ...
    def list_images(self, region=None):
        result = []
        if self.instance_type == 'ec2':
            resp = self.client.describe_images(Filters=[{'Name':'name', 'Values':['ubuntu-16.04*','*centos7*']}])
            result = resp.get("Images",[])
        return result
